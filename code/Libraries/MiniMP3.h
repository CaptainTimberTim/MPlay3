...
typedef float32x4_t minimp3_f4;
static int have_simd()
...
static void L12_read_scale_info(const uint8_t *hdr, bs_t *bs, L12_scale_info *sci)
{
    ...
    {
        sci->scfcod[i] = (uint8_t)(sci->bitalloc[i] ? HDR_IS_LAYER_1(hdr) ? 2 : get_bits(bs, 2) : 6);
    }
...
static void L3_midside_stereo(float *left, int n)
{
    ...
    {
        minimp3_f4 vl = VLD(left + i);
        minimp3_f4 vr = VLD(right + i);
...
static void L3_antialias(float *grbuf, int nbands)
{
    ...
    for (; nbands > 0; nbands--, grbuf += 18)
    {
        ...
        {
            minimp3_f4 vu = VLD(grbuf + 18 + i);
            minimp3_f4 vd = VLD(grbuf + 14 - i);
            minimp3_f4 vc0 = VLD(g_aa[0] + i);
            minimp3_f4 vc1 = VLD(g_aa[1] + i);
            vd = VREV(vd);
...
static void L3_imdct36(float *grbuf, float *overlap, const float *window, int nbands)
{
    ...
    for (j = 0; j < nbands; j++, grbuf += 18, overlap += 9)
    {
        ...
        if (have_simd()) for (; i < 8; i += 4)
        {
            minimp3_f4 vovl = VLD(overlap + i);
            minimp3_f4 vc = VLD(co + i);
            minimp3_f4 vs = VLD(si + i);
            minimp3_f4 vr0 = VLD(g_twid9 + i);
            minimp3_f4 vr1 = VLD(g_twid9 + 9 + i);
            minimp3_f4 vw0 = VLD(window + i);
            minimp3_f4 vw1 = VLD(window + 9 + i);
            minimp3_f4 vsum = VADD(VMUL(vc, vr1), VMUL(vs, vr0));
...
static void mp3d_DCT_II(float *grbuf, int n)
{
    ...
    if (have_simd()) for (; k < n; k += 4)
    {
        ...
        {
            minimp3_f4 x0 = VLD(&y[i*18]);
            minimp3_f4 x1 = VLD(&y[(15 - i)*18]);
            minimp3_f4 x2 = VLD(&y[(16 + i)*18]);
            minimp3_f4 x3 = VLD(&y[(31 - i)*18]);
            minimp3_f4 t0 = VADD(x0, x3);
            minimp3_f4 t1 = VADD(x1, x2);
            minimp3_f4 t2 = VMUL_S(VSUB(x1, x2), g_sec[3*i + 0]);
            minimp3_f4 t3 = VMUL_S(VSUB(x0, x3), g_sec[3*i + 1]);
            x[0] = VADD(t0, t1);
        ...
        {
            minimp3_f4 x0 = x[0], x1 = x[1], x2 = x[2], x3 = x[3], x4 = x[4], x5 = x[5], x6 = x[6], x7 = x[7], xt;
...
#define VSAVE2(i, v) vst1_f32((float32_t *)&y[i*18],  vget_low_f32(v))
            ...
            {
                minimp3_f4 s = VADD(t[3][i], t[3][i + 1]);
                VSAVE2(0, t[0][i]);
...
static void mp3d_synth(float *xl, mp3d_sample_t *dstl, int nch, float *lins)
{
    ...
    {
#define VLOAD(k) minimp3_f4 w0 = VSET(*w++); minimp3_f4 w1 = VSET(*w++); minimp3_f4 vz = VLD(&zlin[4*i - 64*k]); minimp3_f4 vy = VLD(&zlin[4*i - 64*(15 - k)]);
#define _V0(k) { VLOAD(k) b =         VADD(VMUL(vz, w1), VMUL(vy, w0)) ; a =         VSUB(VMUL(vz, w0), VMUL(vy, w1));  }
#define _V1(k) { VLOAD(k) b = VADD(b, VADD(VMUL(vz, w1), VMUL(vy, w0))); a = VADD(a, VSUB(VMUL(vz, w0), VMUL(vy, w1))); }
#define _V2(k) { VLOAD(k) b = VADD(b, VADD(VMUL(vz, w1), VMUL(vy, w0))); a = VADD(a, VSUB(VMUL(vy, w1), VMUL(vz, w0))); }
        minimp3_f4 a, b;
        zlin[4*i]     = xl[18*(31 - i)];
        ...
        _V0(0) _V2(1) _V1(2) _V2(3) _V1(4) _V2(5) _V1(6) _V2(7)
            
        {
#ifndef MINIMP3_FLOAT_OUTPUT
#if HAVE_SSE
            static const minimp3_f4 g_max = { 32767.0f, 32767.0f, 32767.0f, 32767.0f };
            static const minimp3_f4 g_min = { -32768.0f, -32768.0f, -32768.0f, -32768.0f };
            ...
            static const minimp3_f4 g_scale = { 1.0f/32768.0f, 1.0f/32768.0f, 1.0f/32768.0f, 1.0f/32768.0f };
            a = VMUL(a, g_scale);
            b = VMUL(b, g_scale);
#if HAVE_SSE
...
void mp3dec_f32_to_s16(const float *in, int16_t *out, int num_samples)
{
    if(num_samples > 0)
    {
        ...
        {
            static const minimp3_f4 g_scale = { 32768.0f, 32768.0f, 32768.0f, 32768.0f };
            minimp3_f4 a = VMUL(VLD(&in[i  ]), g_scale);
            minimp3_f4 b = VMUL(VLD(&in[i+4]), g_scale);
#if HAVE_SSE
            static const minimp3_f4 g_max = { 32767.0f, 32767.0f, 32767.0f, 32767.0f };
            static const minimp3_f4 g_min = { -32768.0f, -32768.0f, -32768.0f, -32768.0f };
            __m128i pcm8 = _mm_packs_epi32(_mm_cvtps_epi32(_mm_max_ps(_mm_min_ps(a, g_max), g_min)),
